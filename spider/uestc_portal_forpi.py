...
class grade:
    ...
    def send_mail(self, lines, isChange):
        ...
        message['From'] = Header("no-reply@huangjunqin.com", 'utf-8')
        ...
        try:
            ...
            self.runlog("Error: 无法发送邮件")
            return 1

    
    def runlog(self,text):
	if text == '没有新成绩':
	    return False
	else:
	    ISOTIMEFORMAT='%Y-%m-%d %X'
	    fp = open('portal.log','a')
	    fp.write('=========================\n')
	    fp.write(time.strftime( ISOTIMEFORMAT, time.localtime(time.time()))+'\n')
	    fp.write(text+'\n')
	    fp.close()
	    return 0
...
if __name__ == '__main__':
    sender_passwd = '*****'
    grade('receiver@host.com',sender_passwd,'studentID','passwordOfPortal').spider()
